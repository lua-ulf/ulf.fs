...
--- This is ulf.fs
---
--- only minimal changes are made:
--- - fnamemodify(path, ":p:h"): ulf.fs.dirname(ulf.fs.normalize())
--- - replace deps
--- - s/vim\.fs/ulf.fs/gc
---
---@brief]]

---@class ulf.fs.nvim_li
local M = {}

local _splitter = require("ulf.lib.string.splitter")
local startswith = require("ulf.lib.string.affix").startswith
local gsplit = _splitter.gsplit
local _tbl_lib = require("ulf.lib.table.tbl_lib")
local validate = require("ulf.lib.table.validate").validate
...
--- ```lua
--- local root_dir
--- for dir in ulf.fs.parents(vim.api.nvim_buf_get_name(0)) do
...
---            over. The path is first normalized |ulf.fs.normalize()|.
...
function M.dir(path, opts)
	...
	validate("depth", opts.depth, "number", true)
	validate("skip", opts.skip, "function", true)
...
--- local test_dirs = ulf.fs.find(
---   {'test', 'tst', 'testdir'},
---   {limit = math.huge, type = 'directory', path = './runtime/'}
--- )
---
--- -- get all files ending with .cpp or .hpp inside lib/
--- local cpp_hpp = ulf.fs.find(function(name, path)
...
---
---@param opts ulf.fs.find.Opts Optional keyword arguments:
---@return (string[]) # Normalized paths |ulf.fs.normalize()| of all matching items
function M.find(names, opts)
	opts = opts or {}
	validate("names", names, { "string", "table", "function" })
	validate("path", opts.path, "string", true)
	validate("upward", opts.upward, "boolean", true)
	validate("stop", opts.stop, "string", true)
	validate("type", opts.type, "string", true)
	validate("limit", opts.limit, "number", true)
...
--- ulf.fs.root(ulf.fs.joinpath(vim.env.PWD, 'main.py'), {'pyproject.toml', 'setup.py' })
---
--- -- Find the root of a git repository
--- ulf.fs.root(0, '.git')
---
--- -- Find the parent directory containing any file with a .csproj extension
--- ulf.fs.root(0, function(name, path)
...
local function path_resolve_dot(path)
	local is_path_absolute = startswith(path, "/")
	local new_path_components = {}

	for component in gsplit(path, "/") do
...
---@param opts? ulf.fs.normalize.Opts
...
function M.normalize(path, opts)
	...
	if not opts._fast then
		validate("path", path, "string")
		validate("expand_env", opts.expand_env, "boolean", true)
		validate("win", opts.win, "boolean", true)
	end
	...
	if not opts._fast then
		double_slash = startswith(path, "//") and not startswith(path, "///")
	end
