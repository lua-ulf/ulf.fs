...
local tbl = require("ulf.lib.table")
...
describe("#ulf.lib.table", function()
	...
	it("tbl.tbl_extend", function()
		...
		ok((function()
			local a = dict.empty_dict()
			local b = {}
			local c = tbl.tbl_extend("keep", a, b)

			return not types.is_list(c) and tbl.tbl_count(c) == 0
		end)())

		ok((function()
			local a = {}
			local b = dict.empty_dict()
			local c = tbl.tbl_extend("keep", a, b)

			return types.is_list(c) and tbl.tbl_count(c) == 0
		end)())
	...
	it("tbl.tbl_deep_extend", function()
		...
		ok((function()
			...
			return not types.is_list(c) and count == 0
		end)())
		...
		ok((function()
			...
			return types.is_list(c) and count == 0
		end)())
		...
		ok((function()
			...
			local b = { a = { [3] = 3 } }
			local c = tbl.tbl_deep_extend("force", a, b)
			local ret = tbl.deep_equal(c, { a = { [2] = 3, [3] = 3 } })
			return ret
		end)())
		...
		ok((function()
			...
			local b = { sub = { "b", "c" } }
			local c = tbl.tbl_deep_extend("force", a, b)
			return tbl.deep_equal(c, { sub = { "b", "c" } })
		end)())
